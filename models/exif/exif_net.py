...
class EXIFNet():
    ...
    def model(self, preemptive_reuse=False):
                ...
                with tf.device('/gpu:%d' % gpu_id):
                    ...
                    if not self.freeze_base:
                        with tf.compat.v1.name_scope('exif_loss'):
                            loss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=label, logits=out))
                    ...
                    if self.train_classifcation:
                        ...
                        with tf.compat.v1.name_scope('classification_loss'):
                            cls_loss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=cls_label, logits=cls_out))
        ...
        if self.train_classifcation:
            ...
            self.cls_accuracy = tf.reduce_mean(input_tensor=tf.cast(tf.equal(tf.round(self.cls_pred), tf.round(self.cls_label)), tf.float32))
        return
